...
\begin{equation}
   ...
   \ket{
      ...
      -
      (f^{dj}_\triangleright \mkern1mu \psi_d
      +
      f^{dj}_\triangleleft \mkern1mu \psi_d)}
\end{equation}
...
\begin{equation*}
   \sum_i v^a_{\triangleright k} A^{bp}_{\mkern14mu jk} \bra{
      \psi_{b0}
         \psi_{ap} \mkern4mu}
   \ket{
      \mkern2mu
         A^{cqjl} \mkern1mu \psi_{c0} u^{dl}_\triangleright \mkern1mu \psi_{dq}
         +
         A^{cqjl} \mkern1mu \psi_{c0} u^{dl}_\triangleleft \mkern1mu \psi_{dq}
      -
      f^{dj}_{\bowtie} \mkern1mu \psi_{d0}}
\end{equation*}
...
\end{equation}
\begin{equation}
   T_{bapd} = \left\langle \psi_{b0} \psi_{ap} \psi_{d0} \right\rangle \ .
\end{equation}
Tako imamo:
\begin{equation}
   \sum_i v^{ak}_{\triangleright} \left(
      ...
      -
      T_{bapd} A^{bp}_{\mkern14mu jk} f^{dj}_{\bowtie}
   \right) = 0 \ .
\end{equation}
Prekrivne integrale integriramo na referenčnem kvadratu:
\begin{equation}
   S_{bapcdq} = \left\langle \phi^{\phantom{b}0}_b \mkern1mu ( \gimel_{pr} \phi^{\phantom{a}r}_a ) \mkern1mu \phi^{\phantom{c}0}_c ( \gimel_{qs}  \phi^{\phantom{d}s}_d ) \mkern2mu |\mkern-2mu \det \mathbsf{J}| \right\rangle_{\bm{\chi}}
\end{equation}
\begin{equation}
   T_{bapd} = \left\langle \phi^{\phantom{b}0}_b \mkern1mu ( \gimel_{pr} \phi^{\phantom{a}r}_a ) \phi^{\phantom{d}0}_d \mkern2mu |\mkern-2mu \det \mathbsf{J}| \right\rangle_{\bm{\chi}} \ .
\end{equation}
