   ...
   public abstract class ObstructionBlock : TfiBlock {
      ...
      public void WriteLeftHalfOnly(string fileName) {
         ...
         using(StreamWriter sw = new StreamWriter(file.FullName, false)) {
            ...
            sw.Write("{");
               ...
               sw.Write("{");
               for(int k = 0; k < 9; k+=3)
                  sw.Write($"{{{GetNodeStd(i,ColCount - 1,k).ToString()}}}, ");
               sw.Write($"{{{GetNodeStd(i,ColCount - 1,9).ToString()}}}}}");
               sw.WriteLine("}, ");
            }
            sw.Write("{");
            for(int j = ColCount/2; j < ColCount - 1; ++j) {
               sw.Write("{");
               for(int k = 0; k < 9; k+=3)
                  sw.Write($"{{{GetNodeStd(RowCount - 1,j,k).ToString()}}}, ");
               sw.Write($"{{{GetNodeStd(RowCount - 1,j,9).ToString()}}}}}, ");
            }
            sw.Write("{");
            for(int k = 0; k < 9; k+=3)
               sw.Write($"{{{GetNodeStd(RowCount - 1,ColCount - 1,k).ToString()}}}, ");
            sw.WriteLine($"{{{GetNodeStd(RowCount - 1,ColCount - 1,9).ToString()}}}}}}}");
            sw.Write("}");
         }
      }
      ...
      public override bool IsPointInside(ref Pos pos) {
         ...
         for(int row = 0; row < RowCount; ++row) {                              // Add vertices on right edge.
               vertices[counter] = GetNodeStd(row, ColCount - 1, 3)._pos;
               ++counter;
         }
         for(int col = ColCount - 1; col > 0; --col) {                          // Add vertices on upper edge.
               vertices[counter] = GetNodeStd(RowCount - 1, col, 6)._pos;
               ++counter;
         }
