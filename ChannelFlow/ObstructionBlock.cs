...
namespace Fluid.ChannelFlow {   
   ...
   public abstract class ObstructionBlock : Msh.TfiBlock {
      ...
      /// <summary>An extra term needed for computation forcing vector.</summary><remarks></remarks>
      ...
      : base(channelMesh,
         ...
         double rat = channelMesh.GetRelObstructionDiameter();
         double b = channelMesh.GetElementDensity();

         _ChannelMesh = channelMesh;
         _channelFlow = channelFlow;
         _quarterMoonHeight = (1 - 0.5*Sqrt(2))* channelMesh.GetObstructionRadius();
      ...
      public void WriteLeftHalfOnly(string fileName) {
         ...
         using(StreamWriter sw = new StreamWriter(file.FullName, false)) {
               ...
               sw.WriteLine("{");
               for(int i = 0; i < _rowCount - 1; ++i) {                        // Over all rows.
               sw.Write("{");

                  for(int j = _colCount/2; j < _colCount - 1; ++j) {
                     sw.Write("{");
                  ...
                  for(int k = 0; k < 9; k+=3) {
                           sw.Write($"{{{GetNodeStd(i,_colCount - 1,k).ToString()}}}, ");
                  }
                  sw.Write($"{{{GetNodeStd(i,_colCount - 1,9).ToString()}}}}}");
                  sw.WriteLine("}, ");
               }
               sw.Write("{");
               ...
               for(int j = _colCount/2; j < _colCount - 1; ++j) {
                  ...
                  for(int k = 0; k < 9; k+=3) {
                     sw.Write($"{{{GetNodeStd(_rowCount - 1,j,k).ToString()}}}, ");
                  }
                  sw.Write($"{{{GetNodeStd(_rowCount - 1,j,9).ToString()}}}}}, ");
               }
               sw.Write("{");

               for(int k = 0; k < 9; k+=3) {
                     sw.Write($"{{{GetNodeStd(_rowCount - 1,_colCount - 1,k).ToString()}}}, ");
               }
               sw.WriteLine($"{{{GetNodeStd(_rowCount - 1,_colCount - 1,9).ToString()}}}}}}}");
               sw.Write("}");
         }
      }
      ...
      double[][] SubMatrix(int row, int col, int j, int k, double dt, double ni) {
         ...
         A[0][0] = NodeOperatorMatrix0(ref GetNodeStd(row, col, j), dt, ni);
         Transpose(ref A[0][0]);
         A[0][1] = NodeOperatorMatrix1(ref GetNodeStd(row, col, j), dt, ni);
         Transpose(ref A[0][1]);
         A[0][2] = NodeOperatorMatrix1(ref GetNodeStd(row, col, j), dt, ni);
         Transpose(ref A[0][2]);
         A[1] = new double[3][][];                                                   // Create operators for node1.
         A[1][0] = NodeOperatorMatrix0(ref GetNodeStd(row, col, k), dt, ni);
         A[1][1] = NodeOperatorMatrix1(ref GetNodeStd(row, col, k), dt, ni);
         A[1][2] = NodeOperatorMatrix1(ref GetNodeStd(row, col, k), dt, ni);
      ...
      public override bool IsPointInside(ref Pos pos) {
         int nVertices = 2*(_rowCount+_colCount);                                // Construct vertices array.
         var vertices = new Pos[nVertices];
         int counter = 0;

         for(int col = 0; col < _colCount; ++col) {                              // Add vertices on lower edge.
               vertices[counter] = GetNodeStd(0, col, 0)._pos;
               ++counter;
         }
         for(int row = 0; row < _rowCount; ++row) {                              // Add vertices on right edge.
               vertices[counter] = GetNodeStd(row, _colCount - 1, 3)._pos;
               ++counter;
         }
         for(int col = _colCount - 1; col > 0; --col) {                          // Add vertices on upper edge.
               vertices[counter] = GetNodeStd(_rowCount - 1, col, 6)._pos;
               ++counter;
         }
         for(int row = _rowCount - 1; row > 0; --row) {                          // Add vertices on left edge.
               vertices[counter] = GetNodeStd(row, 0, 9)._pos;
               ++counter;
         }
