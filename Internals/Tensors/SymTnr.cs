...
namespace Fluid.Internals.Tensors {
   ...
   public class SymTnr<τ,α> : ValTnr<τ,α>, IEquatable<ValTnr<τ,α>>
...
#nullable enable
      public SymTnr(List<int> structure, (int,int)[] syms, int cap = 6) : base(structure, cap) { }

      public SymTnr(ValTnr<τ,α> sup, (int,int)[] syms, int cap) : base(sup, cap) { }
      ...
      public static SymTnr<dbl,DblA>? CreateAsQuadProd(FTnr? tnr, F2D? det,
      Func<F2D?,dbl> integrate) {
         if(tnr == null || det == null)
            return null;
         return CreateAsQuadProdß(tnr, det, integrate);
      }

      public static SymTnr<dbl,DblA> CreateAsQuadProdß(FTnr tnr, F2D det,
      Func<F2D?,dbl> integrate) {
         var prodTnr = new SymTnr<dbl,DblA>(new List<int> {36,36,36,36},                             // Create product tensor.
      ...
      /// <param name="n">Number of times the constituent appears.</param>
      /// <remarks>For now this is specialized for the case of a rank two tensor.</remarks>
      public static SymTnr<dbl,DblA>? CreateAsTripProd(FTnr? tnr, F2D? det,
      Func<F2D?,dbl> integrate) {
         if(tnr == null || det == null)
            return null;
         return CreateAsTripProdß(tnr, det, integrate);
      }

      public static SymTnr<dbl,DblA> CreateAsTripProdß(FTnr tnr, F2D det,
      Func<F2D?,dbl> integrate) {
         var prodTnr = new SymTnr<dbl,DblA>(new List<int> {36,36,36},                             // Create product tensor.
            new (int,int)[] {(0,1), (0,2), (0,3), (1,2), (1,3), (2,3)}, 36);
         for(int i1 = 0; i1 < 12; ++i1) { for(int j1 = 0; j1 < 3; ++j1) {                          // Over 12 e-nodes, tnr index 1. Over 3 basis funcs at the e-node.
            for(int i2 = i1; i2 < 12; ++i2) { for(int j2 = 0; j2 < 3; ++j2) {
               for(int i3 = i2; i3 < 12; ++i3) { for(int j3 = 0; j3 < 3; ++j3) {
