...
namespace Fluid.Internals.Networks {
   /// <summary>A network is a dictionary of nodes, keyed by unique ID of type τ. Type τ must be a value type.</summary>
   /// <typeparam name="τ">Key type used in internal dictionary.</typeparam>
   public interface INetworkN<τ>
   where τ : notnull {
   /// <summary>Internal dictionary of nodes.</summary>
   Dictionary<τ,INodeV<τ>> Nodes { get; }
   ...
   INodeV<τ>? TrySet(τ key, INodeV<τ>? node) {
      INodeV<τ>? oldNode = TryGet(key);
      if(node != null)
         Nodes[key] = node;
