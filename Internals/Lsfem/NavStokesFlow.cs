   ...
   public abstract class NavStokesFlow : Simulation {
      ...
      protected override void UpdateDynamicsAndForcing() {
         ...
         for(int b = 0; b < NPos; ++b) {                                   // For each position.
            Vec u_j = U(Vec.Ex, b);                                        // Pick a values vector at position a.
            dbl u = u_j[0],
                v = u_j[1],
                p = u_j[2],
                ω = u_j[3],
                invDt = 1/dt;
            Tnr a_qsik = A[Tnr.Ex, b];
            Tnr a_0sik = a_qsik[Tnr.Ex, 0];                                // A0
            Tnr a_00ik = a_0sik[Tnr.Ex, 0];                                   // A00
            a_00ik[1,0] = invDt;  a_00ik[2,1] = invDt;  a_00ik[3,3] = 1;
            Tnr a_01ik = a_0sik[Tnr.Ex, 1];                                   // A01
            a_01ik[1,0] = 0.5*u;  a_01ik[2,0] = 0.5*v;
            Tnr a_02ik = a_0sik[Tnr.Ex, 2];                                   // A02
            a_02ik[1,1] = 0.5*u;
            a_02ik[2,1] = 0.5*v;
            Tnr a_1sik = a_qsik[Tnr.Ex, 1];                                // A1
            Tnr a_10ik = a_1sik[Tnr.Ex, 0];                                   // A10
            a_10ik[0,0] = 1;  a_10ik[1,0] = 0.5*u;  a_10ik[2,1] = 0.5*u;
            a_10ik[2,3] = -0.5*re; a_10ik[3,1] = -1;
            Tnr a_2sik = a_qsik[Tnr.Ex, 2];                                // A2
            Tnr a_20ik = a_2sik[Tnr.Ex, 0];                                   // A20
            a_20ik[0,1] = 1;  a_20ik[1,0] = v;  a_20ik[1,3] = 0.5*re;
            a_20ik[2,1] = v;  a_20ik[2,2] = 1;  a_20ik[3,0] = 1;
            Tnr fs_si = Fs[Tnr.Ex, b];
            Vec fs_0i = fs_si[Vec.Ex, 0];
            fs_0i[1] = u/dt;
            fs_0i[2] = v/dt;
            Vec fs_1i = fs_si[Vec.Ex, 1];
            fs_1i[1] = -0.5*p;
            fs_1i[2] = -0.5*ω/re;
            Vec fs_2i = fs_si[Vec.Ex, 2];
            fs_2i[1] = -0.5*ω/re;
            fs_2i[2] = -0.5*p;
         }
      }
